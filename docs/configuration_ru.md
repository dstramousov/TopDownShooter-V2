...
Задержка перед передачей тревоги другим членам того же отряда.

Если один враг увидел игрока или получил попадание, остальные живые враги
с тем же `spawn_id` становятся тревожными после этой задержки.
