...
## Дополнение: параметры возврата противников домой

### `enemies.return_home_reached_distance_px`

Дистанция в пикселях, на которой противник считается вернувшимся домой.

После достижения этой дистанции противник:

- переходит в состояние `idle`;
- перестаёт быть `alerted`;
- очищает tactical slot;
- очищает текущий path;
- возвращает стартовый угол обзора.

Значение не должно быть слишком маленьким. Если поставить `0`, враг может долго
