...
### lifetime_seconds
Время жизни короткой части impact-эффекта: flash и частиц. Это не время жизни
точки попадания.

### radius_px
Базовый радиус flash/debris-части impact-эффекта.

### material_effects
Настройки визуальных частиц и остаточного hit mark/decal по материалам. Это не
debug-точки, а параметры, из которых рендер собирает короткий flash, разлёт
частиц и маленькую точку/скол попадания.

Для каждого материала задаются:

- `particle_count` — количество частиц;
- `particle_size_px` — диапазон размера частицы;
- `spread_distance_px` — дальность разлёта от центра попадания;
- `burst_intensity` — сила выброса от центра;
- `decal_enabled` — оставляет ли материал точку/след попадания;
- `decal_radius_px` — примерный радиус точки/следа попадания;
- `decal_lifetime_seconds` — время жизни точки/следа попадания.

Decal в первую очередь нужен для ровных и твёрдых поверхностей: камень, металл,
дерево, пол, земля. Для foliage обычно лучше оставить только частицы/листья без
постоянной точки. Минимально должен быть задан `default`.

---

## shell_ejection

Настройки визуального выброса гильз. На каждый spawned shot создаётся одна гильза.
Гильзы не участвуют в collision и не влияют на gameplay.

### enabled
Включает выброс гильз.

### shell_size_px
Диапазон размера гильзы в пикселях.

### lifetime_seconds
Диапазон времени жизни гильзы.

### ejection_distance_px
Базовая дальность вылета гильзы от оружия.

### ejection_intensity
Множитель силы выброса.

### spread_degrees
Угловой разброс вокруг направления выброса вправо от оружия.

### max_active_shells
Защитный лимит активных гильз в памяти.
...
Максимальное количество A* перестроений пути за один update.
...
По умолчанию выключено: текущая реализация дорогая и может давать заметные spike-и при массовом агре.
...
#### fire_enabled
Включает стрельбу engaged-врагов по игроку.

#### fire_primary_weapon_id
ID основного оружия врага из общей базы `res/config/weapons.json`. По умолчанию `ak47`.

#### fire_fallback_weapon_id
ID резервного оружия врага из общей базы `res/config/weapons.json`. По умолчанию `pistol`. Враг переходит на него, когда основной автоматический ствол полностью пуст: магазин и резерв равны нулю.

#### fire_max_distance_px
Максимальная дистанция, на которой враг вообще имеет право открыть огонь. Дальность самой пули берётся из выбранного оружия.

#### fire_muzzle_offset_px
Смещение muzzle-точки от центра врага вперёд по направлению выстрела.

#### fire_aim_error_degrees
Дополнительная ошибка наведения врага. Она добавляется поверх `spread_degrees` выбранного оружия. Урон, темп стрельбы, магазин, резерв, reload time, tracer lifetime и радиус выстрела берутся из общей базы оружия.
...
В v0.2.15 зарезервировано для следующих итераций.

### group_size_min / group_size_max
Границы размера будущих spawn-групп.
В v0.2.15 стартовый спавн создаёт одиночные маркеры; группы будут подключаться отдельно.
