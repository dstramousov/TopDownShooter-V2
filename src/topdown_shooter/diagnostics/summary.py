...
def build_inspection_summary(package: GeneratedMapPackage, runtime_map: RuntimeMap) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- enemy spawn candidate tiles: {len(runtime_map.enemy_spawn_candidate_tiles)}",
            f"- loot candidate tiles: {len(runtime_map.loot_candidate_tiles)}",
            f"- safe tiles: {len(runtime_map.safe_tiles)}",
            f"- extraction tiles: {len(runtime_map.extraction_tiles)}",
