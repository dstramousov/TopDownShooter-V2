...
class EnemySystem:
    ...
    @classmethod
    def from_runtime_spawn_sources(
        cls,
        *,
        tactical_map: dict[str, object],
        runtime_map: RuntimeMap,
        ...
        """Create the startup enemy system from the best available spawn source.

        Structured maps with zone-driven candidates use ``SpawnDirector`` for
        initial placement. Legacy maps without gameplay zones keep using the
        tactical ``enemy_spawn_zones`` path unchanged.

        Args:
            tactical_map: Raw tactical map dictionary loaded from the map package.
            runtime_map: Runtime map used for tile-size conversion and bounds checks.
            player_tile: Current player tile used by zone-driven spawn filters.
            enemy_spawn_config: Zone-driven spawn configuration.
            enemy_max_health: Initial and maximum health for spawned enemies.
            hit_marker_lifetime_seconds: Enemy hit marker lifetime in seconds.
            hit_marker_radius_px: Enemy hit marker radius in world pixels.
            smart_facing_enabled: Whether missing spawn facing uses map-aware scoring.
            ...
            facing_wall_penalty_distance_px: Distance used to penalize near-wall facing.
            facing_probe_step_px: Sampling step for facing probe rays.
            min_squad_size: Legacy minimum enemies generated by one spawn zone.
            max_squad_size: Legacy maximum enemies generated by one spawn zone.
            squad_radius_px: Legacy squad placement radius.
            min_enemy_spacing_px: Legacy minimum distance between initially placed enemies.
            max_initial_enemies: Legacy global cap for tactical-map startup enemies.
            placement_attempts_per_enemy: Legacy candidate attempts for each squad member.
            spawn_collision_radius_px: Collision radius used to validate legacy spawn positions.

        Returns:
            Enemy system populated from zone-driven candidates or legacy tactical spawns.
        """
        if runtime_map.enemy_spawn_candidate_tiles:
            director = SpawnDirector(runtime_map, enemy_spawn_config)
            return cls.from_spawn_director(
        ...
        return cls.from_tactical_map(
            ...
            squad_radius_px=squad_radius_px,
            min_enemy_spacing_px=min_enemy_spacing_px,
            max_initial_enemies=max_initial_enemies,
            placement_attempts_per_enemy=placement_attempts_per_enemy,
            spawn_collision_radius_px=spawn_collision_radius_px,
        )

    @classmethod
    def from_spawn_director(
        cls,
        *,
    ...
    def _select_spawn_metadata_zone(runtime_map: RuntimeMap, tile: TileCoord) -> object | None:
        ...
        preferred_types = {"spawn_area", "encounter_area", "ambush_area", "danger_area"}
        fallback_zone = None
        for zone in runtime_map.zones_at_tile(tile):
            if fallback_zone is None:
                fallback_zone = zone
            if zone.zone_type in preferred_types:
                return zone
        return fallback_zone

    @staticmethod
    def _count_zone_spawn_sources(runtime_map: RuntimeMap) -> int:
        """Return the number of gameplay zones that can feed initial spawn."""
        preferred_types = {"spawn_area", "encounter_area", "ambush_area", "danger_area"}
        return sum(
            1
            for zone in runtime_map.gameplay_zones
            if zone.zone_type in preferred_types
        )
